...
uint8_t GC_ExecuteLine(char *line)
{
	...
	while(line[char_counter] != 0)  // Loop until no more g-code words in line.
    {
		...
		case 'M':
            ...
            case 101:
                word_bit = MODAL_GROUP_M101;
                gc_block.modal.prim_grip = RETRACT;
                break;
            case 102:
                word_bit = MODAL_GROUP_M101;
                gc_block.modal.prim_grip = EXTEND;
                break;
            case 103:
                word_bit = MODAL_GROUP_M103;
                gc_block.modal.sec_grip = RETRACT;
                break;
            case 104:
                word_bit = MODAL_GROUP_M103;
                gc_block.modal.sec_grip = EXTEND;
                break;
            case 105:
                word_bit = MODAL_GROUP_M105;
                gc_block.modal.shot_pin = RETRACT;
                break;
            case 106:
                word_bit = MODAL_GROUP_M105;
                gc_block.modal.shot_pin = EXTEND;
                break;
#endif
	...
	*/
	...
	// [8.5 Pneumatic control]: Fire appropriate cylinder

#ifdef ENABLE_PNEUMATICS

    if (gc_state.modal.prim_grip != gc_block.modal.prim_grip)
    {
        gc_state.modal.prim_grip = gc_block.modal.prim_grip;
        if(gc_block.modal.prim_grip ==  EXTEND){
            Extend_Cylinder(0);
        } else {
            Retract_Cylinder(0);
        }
    ...
    if (gc_state.modal.sec_grip != gc_block.modal.sec_grip)
    {
        gc_state.modal.sec_grip = gc_block.modal.sec_grip;
        if(gc_block.modal.sec_grip ==  EXTEND){
            Extend_Cylinder(1);
        } else {
            Retract_Cylinder(1);
        }
    ...
    if (gc_state.modal.shot_pin != gc_block.modal.shot_pin)
    {
        gc_state.modal.shot_pin = gc_block.modal.shot_pin;
        if(gc_block.modal.shot_pin ==  EXTEND){
            Extend_Cylinder(2);
        } else {
            Retract_Cylinder(2);
        }
