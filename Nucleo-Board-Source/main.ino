...
float *getSpeed(float feedrate, float delta[NUM_AXIES], float speed[NUM_AXIES])     //Takes feedrate & array of z, a, x, delta values
...
float calcDecel(float current_step_speed[NUM_AXIES], float next_step_speed[NUM_AXIES], float max_step_accel[NUM_AXIES]){
    //Takes current and next speeds in steps/s. Calculates how much speeds must be slowed such that transisitioning from
    //current_speed * decel to next_speed * decel does not exceed max acceleration for any axis
...
int stepsToAccelerate(float current_step_speed[NUM_AXIES], float next_step_speed[NUM_AXIES], float max_step_accel[NUM_AXIES], int limitingAxis){
//Calculates number of steps to accelerate from current speed to next speed
//given the maximum acceleration of each axis. Velocity vectors MUST be colinear.
    int numSteps = 0;
    
    numSteps = ceil(abs(sq(current_step_speed[limitingAxis]) - sq(next_step_speed[limitingAxis]))/(2*max_step_accel[i]));
    return numSteps;
}
